...
description: >-
  Use when opening or updating a GitHub pull request that needs screenshots,
  videos, or proof artifacts rendered inline in the PR description instead of
  shown as plain links.
---
...
open the PR and see the evidence where the template asks for it, in the
`Behavioural Proof (with video and screenshots)` section of
...
Confirm the proof section contains inline image Markdown. The machine gate
(`scripts/validate-pr-body.mjs`, run by the `PR quality` workflow) enforces the
same rule: the proof section must contain `![` or the exact string
`Not applicable`. If the change has no rendered or behavioural proof surface,
write `Not applicable` in the proof section with the technical reason.
