...
#   • Ponytail          — shortest-working-diff discipline
#
...
bold "1/6  Fallow (code intelligence)"
...
bold "3/6  ArchitectPlaybook (repo health-check audits)"
...
bold "4/6  graphify (knowledge graph)"
...
bold "5/6  GitNexus (execution-flow index)"
...
bold "6/6  Ponytail (simplicity discipline)"
if [ -d "${HOME}/.codex/plugins/cache/ponytail" ]; then
  ok "Ponytail plugin present in Codex plugin cache"
else
  note "Ponytail not found — install the Ponytail Codex plugin before relying on repo Simplicity Mode"
fi
